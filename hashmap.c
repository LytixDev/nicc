...
static inline u8 hm_hash_extra(u32 hash) { return (u8)(hash & ((1 << 8) - 1)); }
...
static inline void insert_entry(struct hm_entry_t *found,
                                struct hm_entry_t *new) {
...
static int insert(struct hm_bucket_t *bucket, struct hm_entry_t *new) {
  ...
  for (u8 i = 0; i < HM_BUCKET_SIZE; i++) {
    ...
    if (entry->key != NULL && new->hash_extra ==
                                  entry->hash_extra &&new->key_size ==
                                  entry->key_size) {
...
static struct hm_entry_t *get_from_bucket(struct hm_bucket_t *bucket, void *key,
                                          u32 key_size, u8 hash_extra) {
...
static struct hm_entry_t *get_entry(struct hashmap_t *map, void *key,
                                    u32 key_size) {
  if (map->len == 0)
    return NULL;
...
static void re_insert(u32 size_log2, struct hm_bucket_t *buckets,
                      struct hm_entry_t *entry) {
...
static void increase(struct hashmap_t *map) {
  ...
  struct hm_bucket_t *new_buckets =
      malloc(sizeof(struct hm_bucket_t) * n_buckets);
...
void hashmap_put(struct hashmap_t *map, void *key, u32 key_size, void *value,
                 u32 val_size, bool alloc_flag) {
  double load_factor =
      (double)map->len / (N_BUCKETS(map->size_log2) * HM_BUCKET_SIZE);
